  ...
  useEffect(() => {
    const nextFormData = museum
      ? {
        ...
        virtual_tour_url: museum.virtual_tour_url || '',
        live_cam_url: museum.live_cam_url || '',
      }
      : {
        nama_museum: '',
        latitude: '',
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">URL Virtual Tour 360°</label>
            <input
              type="url"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">URL Live Cam (YouTube/HLS)</label>
            <input
              type="url"
              value={formData.live_cam_url}
              onChange={(e) => handleChange('live_cam_url', e.target.value)}
              placeholder="https://www.youtube.com/watch?v=..."
              className={inputClass('live_cam_url')}
            />
          </div>
        </div>
