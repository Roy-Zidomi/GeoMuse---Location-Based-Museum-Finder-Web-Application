...
const AboutPlatform = () => {
  ...
  const content = {
    id: {
      badge: 'Visi Platform',
      titlePrefix: 'Cara Baru Menjelajah',
      titleHighlight: 'Warisan Kita',
      description:
        'Kami membangun platform paling komprehensif untuk memetakan dan mengeksplorasi sebaran museum di Indonesia. Saat ini sebagai pusat informasi, ke depan fitur kami akan mempermudah perencanaan perjalanan budaya kamu.',
      floatingMuseumName: 'Museum Fatahillah',
      floatingDistance: '0.5 km dari sini',
      floatingTagCategory: 'Seni',
      floatingTagRegion: 'DKI Jakarta',
      features: [
        {
          icon: <Filter className="w-6 h-6" />,
          title: 'Filter Cerdas',
          description: 'Cari museum berdasarkan provinsi, kabupaten/kota, atau kategori.',
        },
        {
          icon: <Navigation className="w-6 h-6" />,
          title: 'Berbasis Lokasi',
          description: 'Temukan museum terdekat berdasarkan lokasi kamu secara instan.',
        },
        {
          icon: <Map className="w-6 h-6" />,
          title: 'Peta Interaktif',
          description: 'Eksplorasi visual sebaran museum di seluruh Indonesia.',
        },
        {
          icon: <Compass className="w-6 h-6" />,
          title: 'Panduan Detail',
          description: 'Dapatkan info tiket, jam buka, dan highlight koleksi.',
        },
      ],
    },
    en: {
      badge: 'Platform Vision',
      titlePrefix: 'A New Way to Discover',
      titleHighlight: 'Our Heritage',
      description:
      ...
      floatingMuseumName: 'Museum Fatahillah',
      floatingDistance: '0.5 km away',
      floatingTagCategory: 'Art',
      floatingTagRegion: 'DKI Jakarta',
      features: [
        {
          icon: <Filter className="w-6 h-6" />,
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-panel mb-6 bg-indigo-50 dark:bg-indigo-900/30">
              <span className="text-indigo-600 dark:text-indigo-400 font-bold text-xs uppercase tracking-widest">{text.badge}</span>
            </div>
            
            <h2 className="text-3xl md:text-5xl font-bold text-slate-900 dark:text-white mb-6 leading-tight">
              {text.titlePrefix} <br/> <span className="museum-gradient-text">{text.titleHighlight}</span>
            </h2>
            ...
            <div className="aspect-square md:aspect-[4/3] rounded-3xl bg-slate-900 overflow-hidden shadow-2xl relative border border-slate-800">
              ...
              <div className="p-8 pt-16 h-full flex items-center justify-center relative overflow-hidden">
                ...
                >
                  ...
                  <div>
                    <p className="text-xs font-bold text-slate-900 dark:text-white">{text.floatingMuseumName}</p>
                    <p className="text-[10px] text-slate-500">{text.floatingDistance}</p>
                  </div>
                </MotionDiv>
                ...
                >
                  <div className="flex gap-2">
                    <span className="px-3 py-1 bg-indigo-100 text-indigo-600 rounded-full text-xs font-bold">{text.floatingTagCategory}</span>
                    <span className="px-3 py-1 bg-emerald-100 text-emerald-600 rounded-full text-xs font-bold">{text.floatingTagRegion}</span>
                  </div>
                </MotionDiv>
              </div>
            </div>
